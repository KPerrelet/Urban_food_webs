...
for(i in 1:nrow(bg.df)){ 
  ...
  for (j in c(1:100)) { # 100 iterations as the cluster_louvain algorithm is heuristic
    ...
    N_Clust <- length(unique(membership(Community_rep)))
  ...
  for (j in c(1:100)) {
    ...
    Skew.Degree.rand <- append(Skew.Degree.rand, skewness(igraph::degree(graph.adjacency(MW.local.rand, mode = "directed"))))
    Nic.over.Con.Jac.rand = append(Nic.over.Con.Jac.rand, networklevel(MW.local.rand, index = "niche overlap", dist = "jaccard")[1]) # viewed as consumers vs. resources but actually the same set of taxa.
    Mean.Gen.rand = append(Mean.Gen.rand, mean(apply(MW.local.rand, 2, sum)))
    
    TL.info.rand = Unw.TL.Omn.q(MW.local.rand, ncol(MW.local.rand), which(apply(MW.local.rand, 2, sum)==0))
    Mean.TL.rand = append(Mean.TL.rand, mean(TL.info.rand$TL))
    Omnivory.rand = append(Omnivory.rand, mean(TL.info.rand$Omn, na.rm = T)) # In terms of TL, not primary producer
    Coherence.rand = append(Coherence.rand, TL.info.rand$q) # Lower q, higher trophic coherence.
    Basal.rand = append(Basal.rand, length(TL.info.rand$TL[TL.info.rand$TL <= 2 & TL.info.rand$TL > 1]))
    Predator.rand = append(Predator.rand, length(TL.info.rand$TL[TL.info.rand$TL > 2]))
    
    Nestedness.rand = append(Modularity.rand, unname(nestednodf(MW.local.rand)$statistic)[3])
    
    Size.Cluster.BG.rand.it <- c()
    Frac.Cluster.BG.rand.it <- c()
    Modularity.rand.it <- c()
    for (i in c(1:100)) {
      Community.rand <- cluster_louvain(graph_from_adjacency_matrix(MW.local.rand, mode = "undirected"))
      Modularity.rand.it <- append(Modularity.rand.it, modularity(Community.rand))
      ...
      Size.Cluster.BG.rand.it <- append(Size.Cluster.BG.rand.it, 
                                        length(ID.Cluster.BG.rand) - length(which(names(ID.Cluster.BG.rand) %in% resources)))
      Frac.Cluster.BG.rand.it <- append(Frac.Cluster.BG.rand.it, 
                                        (length(ID.Cluster.BG.rand) - length(which(names(ID.Cluster.BG.rand) %in% resources))) / Sp.Richness)
    }
    
    Modularity.rand <- append(Modularity.rand, mean(Modularity.rand.it))
    Size.Cluster.BG.rand <- append(Size.Cluster.BG.rand, mean(Size.Cluster.BG.rand.it))
    Frac.Cluster.BG.rand <- append(Frac.Cluster.BG.rand, mean(Frac.Cluster.BG.rand.it))
    ...
    paths <- distances(graph_from_adjacency_matrix(MW.local.rand[-which(rownames(MW.local.rand) %in% resources),
                       ...
                       mode = "out")
    paths[is.infinite(paths) == T] <- max(paths[is.infinite(paths) == F]) + 1
    Mean.ShortestPath.rand <- append(Mean.ShortestPath.rand, mean(paths))
  }
  Basal.rand <- mean(Basal.rand)
  Predator.rand <- mean(Predator.rand)
  Skew.Degree.rand <- mean(Skew.Degree.rand)
  Nic.over.Con.Jac.rand <- mean(Nic.over.Con.Jac.rand)
  Mean.Gen.rand <- mean(Mean.Gen.rand)
  Mean.TL.rand <- mean(Mean.TL.rand)
  Omnivory.rand <- mean(Omnivory.rand)
  Coherence.rand <- mean(Coherence.rand)
  Nestedness.rand <- mean(Nestedness.rand)
  ...
  Mean.ShortestPath.rand <- mean(Mean.ShortestPath.rand)
